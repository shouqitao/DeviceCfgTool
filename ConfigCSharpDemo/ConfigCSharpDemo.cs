...
namespace ConfigCSharpDemo {
    public partial class ConfigCSharpDemo : Form {
        private readonly bool m_bInitSDK;
        ...
        public void GetDevChanList() {
            ...
            var dwDChanTotalNum = m_struDeviceInfo.byIPChanNum + 256 * (uint)m_struDeviceInfo.byHighDChanNum;

            if (dwDChanTotalNum > 0) {
                var dwSize = (uint)Marshal.SizeOf(m_struIpParaCfgV40);
                ...
                if (!CHCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_GET_IPPARACFG_V40, iGroupNo,
                    ...
                    m_struIpParaCfgV40 =
                        (CHCNetSDK.NET_DVR_IPPARACFG_V40)Marshal.PtrToStructure(ptrIpParaCfgV40,
                            typeof(CHCNetSDK.NET_DVR_IPPARACFG_V40));
                    ...
                    for (i = 0; i < iDChanNum; i++) {
                        ...
                        var m_struStreamMode = new CHCNetSDK.NET_DVR_STREAM_MODE();
                        ...
                        switch (byStreamType) {
                            ...
                            case 0:
                                ...
                                Marshal.StructureToPtr(m_struIpParaCfgV40.struStreamMode[i].uGetStream, ptrChanInfo,
                                    false);
                                var m_struChanInfo = new CHCNetSDK.NET_DVR_IPCHANINFO();
                                m_struChanInfo =
                                    (CHCNetSDK.NET_DVR_IPCHANINFO)Marshal.PtrToStructure(ptrChanInfo,
                                        typeof(CHCNetSDK.NET_DVR_IPCHANINFO));
                            ...
                            case 6:
                                ...
                                Marshal.StructureToPtr(m_struIpParaCfgV40.struStreamMode[i].uGetStream, ptrChanInfoV40,
                                    false);
                                var m_struChanInfoV40 = new CHCNetSDK.NET_DVR_IPCHANINFO_V40();
                                m_struChanInfoV40 =
                                    (CHCNetSDK.NET_DVR_IPCHANINFO_V40)Marshal.PtrToStructure(ptrChanInfoV40,
                                        typeof(CHCNetSDK.NET_DVR_IPCHANINFO_V40));
        ...
        private void btnLogin_Click(object sender, EventArgs e) {
            ...
            var DVRIPAddress = textBoxIP.Text;
            var DVRPortNumber = short.Parse(textBoxPort.Text);
            var DVRUserName = textBoxUserName.Text;
            var DVRPassword = textBoxPassword.Text;

            //登录设备 Login the device
            m_lUserID = CHCNetSDK.NET_DVR_Login_V30(DVRIPAddress, DVRPortNumber, DVRUserName, DVRPassword,
                ref m_struDeviceInfo);
            if (m_lUserID == -1) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
        ...
        private void btnLogout_Click(object sender, EventArgs e) {
            ...
            if (!CHCNetSDK.NET_DVR_Logout(m_lUserID)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                strErr = "NET_DVR_Logout failed, error code= " +
                         iLastErr; //注销登录失败，输出错误号 Failed to logout and output the error code
                MessageBox.Show(strErr);
            } else {
                m_lUserID = -1;
                ParameterClear();
        ...
        private void btnReboot_Click(object sender, EventArgs e) {
            ...
            //重启设备 Reboot the device           
            if (!CHCNetSDK.NET_DVR_RebootDVR(m_lUserID)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                strErr = "NET_DVR_RebootDVR failed, error code= " +
                         iLastErr; //设备重启失败，输出错误号 Failed to reboot and output the error code
                MessageBox.Show(strErr);
            } else {
                m_lUserID = -1;
                ParameterClear();
        ...
        private void btnDevCfg_Click(object sender, EventArgs e) {
            var byName = Encoding.Default.GetBytes(textBoxDevName.Text);
            m_struDeviceCfg.sDVRName = new byte[32];
            byName.CopyTo(m_struDeviceCfg.sDVRName, 0);

            var nSize = Marshal.SizeOf(m_struDeviceCfg);
            var ptrDeviceCfg = Marshal.AllocHGlobal(nSize);
            Marshal.StructureToPtr(m_struDeviceCfg, ptrDeviceCfg, false);

            if (!CHCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_SET_DEVICECFG_V40, -1, ptrDeviceCfg,
                (uint)nSize)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
        ...
        private void btnDevCfgGet_Click(object sender, EventArgs e) {
            ...
            var ptrDeviceCfg = Marshal.AllocHGlobal(nSize);
            Marshal.StructureToPtr(m_struDeviceCfg, ptrDeviceCfg, false);
            if (!CHCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_GET_DEVICECFG_V40, -1, ptrDeviceCfg,
                ...
                textBoxDevName.Text = Encoding.GetEncoding("GBK").GetString(m_struDeviceCfg.sDVRName);
                textBoxDevType.Text = Encoding.UTF8.GetString(m_struDeviceCfg.byDevTypeName);
                textBoxANum.Text = Convert.ToString(m_struDeviceCfg.byChanNum);
                textBoxIPNum.Text =
                    Convert.ToString(m_struDeviceCfg.byIPChanNum + 256 * m_struDeviceCfg.byHighIPChanNum);
                ...
                textBoxDevSerial.Text = Encoding.UTF8.GetString(m_struDeviceCfg.sSerialNumber);

                var iVer1 = (m_struDeviceCfg.dwSoftwareVersion >> 24) & 0xFF;
                var iVer2 = (m_struDeviceCfg.dwSoftwareVersion >> 16) & 0xFF;
                var iVer3 = m_struDeviceCfg.dwSoftwareVersion & 0xFFFF;
                var iVer4 = (m_struDeviceCfg.dwSoftwareBuildDate >> 16) & 0xFFFF;
                var iVer5 = (m_struDeviceCfg.dwSoftwareBuildDate >> 8) & 0xFF;
                var iVer6 = m_struDeviceCfg.dwSoftwareBuildDate & 0xFF;

                textBoxDevVersion.Text = "V" + iVer1 + "." + iVer2 + "." + iVer3 + " Build " +
                                         $"{iVer4:D2}" + $"{iVer5:D2}" +
                                         $"{iVer6:D2}";
            }
        ...
        private void btnNetCfgGet_Click(object sender, EventArgs e) {
            uint dwReturn = 0;
            var nSize = Marshal.SizeOf(m_struNetCfg);
            var ptrNetCfg = Marshal.AllocHGlobal(nSize);
            Marshal.StructureToPtr(m_struNetCfg, ptrNetCfg, false);

            if (!CHCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_GET_NETCFG_V30, -1, ptrNetCfg,
                (uint)nSize, ref dwReturn)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                ...
                m_struNetCfg =
                    (CHCNetSDK.NET_DVR_NETCFG_V30)Marshal.PtrToStructure(ptrNetCfg,
                ...
                if (m_struNetCfg.struPPPoE.dwPPPOE == 1) {
                    ...
                    textBoxPPPoeName.Text = Encoding.UTF8.GetString(m_struNetCfg.struPPPoE.sPPPoEUser);
        ...
        private void btnNetCfgSet_Click(object sender, EventArgs e) {
            ...
            m_struNetCfg.wHttpPortNo = ushort.Parse(textBoxHttpCfg.Text);
            m_struNetCfg.struEtherNet[0].wDVRPort = ushort.Parse(textBoxSdkCfg.Text);
            m_struNetCfg.struAlarmHostIpAddr.sIpV4 = textBoxHostIP.Text;
            m_struNetCfg.wAlarmHostIpPort = ushort.Parse(textBoxHostPort.Text);
            ...
            if (checkBoxPPPoe.Checked) {
                ...
                var byName = Encoding.Default.GetBytes(textBoxPPPoeName.Text);
            ...
            if (!CHCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_SET_NETCFG_V30, -1, ptrNetCfg,
                (uint)nSize)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
        ...
        private void btnTimeGet_Click(object sender, EventArgs e) {
            uint dwReturn = 0;
            var nSize = Marshal.SizeOf(m_struTimeCfg);
            var ptrTimeCfg = Marshal.AllocHGlobal(nSize);
            Marshal.StructureToPtr(m_struTimeCfg, ptrTimeCfg, false);
            if (!CHCNetSDK.NET_DVR_GetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_GET_TIMECFG, -1, ptrTimeCfg, (uint)nSize,
                ref dwReturn)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                ...
                m_struTimeCfg =
                    (CHCNetSDK.NET_DVR_TIME)Marshal.PtrToStructure(ptrTimeCfg, typeof(CHCNetSDK.NET_DVR_TIME));
        ...
        private void btnTimeSet_Click(object sender, EventArgs e) {
            m_struTimeCfg.dwYear = uint.Parse(textBoxYear.Text);
            m_struTimeCfg.dwMonth = uint.Parse(textBoxMonth.Text);
            m_struTimeCfg.dwDay = uint.Parse(textBoxDay.Text);
            m_struTimeCfg.dwHour = uint.Parse(textBoxHour.Text);
            m_struTimeCfg.dwMinute = uint.Parse(textBoxMinute.Text);
            m_struTimeCfg.dwSecond = uint.Parse(textBoxSecond.Text);

            var nSize = Marshal.SizeOf(m_struTimeCfg);
            var ptrTimeCfg = Marshal.AllocHGlobal(nSize);
            Marshal.StructureToPtr(m_struTimeCfg, ptrTimeCfg, false);

            if (!CHCNetSDK.NET_DVR_SetDVRConfig(m_lUserID, CHCNetSDK.NET_DVR_SET_TIMECFG, -1, ptrTimeCfg,
                (uint)nSize)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
        ...
        private void btnXML_Click(object sender, EventArgs e) {
            var pInputXml = new CHCNetSDK.NET_DVR_XML_CONFIG_INPUT();
            var nInSize = Marshal.SizeOf(pInputXml);
            pInputXml.dwSize = (uint)nInSize;

            var strRequestUrl = textBoxRequest.Text;
            var dwRequestUrlLen = (uint)strRequestUrl.Length;
            pInputXml.lpRequestUrl = Marshal.StringToHGlobalAnsi(strRequestUrl);
            pInputXml.dwRequestUrlLen = dwRequestUrlLen;

            var strInputParam = textBoxInXML.Text;

            pInputXml.lpInBuffer = Marshal.StringToHGlobalAnsi(strInputParam);
            pInputXml.dwInBufferSize = (uint)strInputParam.Length;

            var pOutputXml = new CHCNetSDK.NET_DVR_XML_CONFIG_OUTPUT();
            ...
            var strOutputParam = Marshal.PtrToStringAnsi(pOutputXml.lpOutBuffer);
        ...
        private void btnTest_Click(object sender, EventArgs e) {
            ...
            */
            ...
            var struConfigParam = new CHCNetSDK.NET_DVR_STD_CONFIG();
            uint dwChannel = 2;
            ...
            var struManulThermometryBasic = new CHCNetSDK.NET_SDK_MANUALTHERM_BASICPARAM();
            var nOutSize = Marshal.SizeOf(struManulThermometryBasic);
            struConfigParam.lpOutBuffer = Marshal.AllocHGlobal(nOutSize);
            struConfigParam.dwOutSize = (uint)nOutSize;

            if (!CHCNetSDK.NET_DVR_GetSTDConfig(m_lUserID, CHCNetSDK.NET_DVR_GET_MANUALTHERM_BASICPARAM,
                ref struConfigParam)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                strErr = "NET_DVR_GET_THERMOMETRY_ALARMRULE failed, error code= " + iLastErr;
            ...
            struManulThermometryBasic =
                (CHCNetSDK.NET_SDK_MANUALTHERM_BASICPARAM)Marshal.PtrToStructure(struConfigParam.lpOutBuffer,
                    typeof(CHCNetSDK.NET_SDK_MANUALTHERM_BASICPARAM));
            struManulThermometryBasic.wDistance = 20;
            struConfigParam.lpInBuffer = Marshal.AllocHGlobal(nOutSize);
            struConfigParam.dwInSize = (uint)nOutSize;
            Marshal.StructureToPtr(struManulThermometryBasic, struConfigParam.lpInBuffer, false);

            if (!CHCNetSDK.NET_DVR_SetSTDConfig(m_lUserID, CHCNetSDK.NET_DVR_SET_MANUALTHERM_BASICPARAM,
                ref struConfigParam)) {
                iLastErr = CHCNetSDK.NET_DVR_GetLastError();
            ...
            //手动测温规则配置
            var struManualTherm = new CHCNetSDK.NET_SDK_MANUAL_THERMOMETRY();
            var iInSize = Marshal.SizeOf(struManualTherm);
            ...
            var byName = Encoding.Default.GetBytes("TestName");
